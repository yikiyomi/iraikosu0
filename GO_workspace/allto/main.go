...
func main() {
	...
	auth.Use(AuthMiddleware())
	{
		...
		auth.GET("/posts/:id/likes", ListPostLikes)     //获取帖子的点赞列表
		auth.GET("/posts/likes", ListMyLikes)           //当前用户点赞的帖子列表
		auth.GET("/posts/:id/likers", ListPostLikers)   //帖子点赞用户列表
	}
...
func ListMyLikes(c *gin.Context) {
	userID := c.GetUint("userID")
	var likes []Like
	if err := db.Preload("Post").Where("user_id=?", userID).Order("created_at desc").Find(&likes).Error; err != nil {
...
func ListPostLikes(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "无效帖子id"})
		return
	}
	var likes []Like
	if err := db.Where("post_id = ?", postID).Order("created_at desc").Find(&likes).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "查询点赞的用户列表失败"})
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": likes})
...
func ListPostLikers(c *gin.Context) {
	...
	// 使用JOIN查询关联表
	var likers []User
	if err := db.Joins("JOIN likes ON users.id = likes.user_id").
